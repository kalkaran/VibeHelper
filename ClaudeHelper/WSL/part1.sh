...
WSL_BASE_PACKAGES=(curl git ca-certificates python3 python3-pip python3-venv nodejs npm)
WSL_OPTIONAL_PACKAGES=(pipx)
...
Options:
  ...
  --no-system-packages
                  Skip Ubuntu apt package bootstrap.
...
PLATFORM_LABEL="${CLAUDE_HELPER_PLATFORM_LABEL:-WSL}"
...
early_sudo_prefix() {
	...
	warn "sudo is required for apt installs when not running as root."
	return 1
}

early_install_system_packages() {
	if [[ "$SKIP_SYSTEM_PACKAGES" -eq 1 ]]; then
		log "Skipping apt package bootstrap."
		return 0
	fi
	if ! command -v apt-get >/dev/null 2>&1; then
		warn "apt-get not found; skipping system package bootstrap. This script supports Ubuntu WSL."
		return 0
	fi
	...
	if ! DEBIAN_FRONTEND=noninteractive NEEDRESTART_MODE=a "${sudo_cmd[@]}" apt-get update; then
...
maybe_install_rtk() {
	...
	warn "curl is unavailable; the routing hook no-ops until RTK is installed."
	return 0
}

npx_ready() {
	if ! command -v npx >/dev/null 2>&1; then
		warn "npx not found; skipping npm-based setup."
		return 1
	fi
	local major
	major="$(node -p 'Number(process.versions.node.split(".")[0])' 2>/dev/null || true)"
	if [[ ! "$major" =~ ^[0-9]+$ ]]; then
		warn "Could not determine Node.js version; skipping npm-based setup."
		return 1
	fi
	if [[ "$major" -lt 18 ]]; then
		warn "Node.js >=18 is required for npm-based setup; found major version $major. Install a newer Node.js, then rerun."
		return 1
	fi
	return 0
}
...
ensure_prereqs() {
	...
	warn "Neither uv nor pipx is available; graphify and code-review-graph installs may fall back to python3 -m pip --user."
}
...
setup_chrome_mcp() {
	...
	if [[ "$state" == "missing" ]] && chrome_mcp_claude_plugin_installed; then
		...
		if ! chrome_browser_available; then
			warn "Linux Chrome was not detected in WSL. Install current stable Chrome inside WSL or configure Chrome DevTools MCP with an explicit browser connection before use."
		fi
		return 0
	fi
	...
	if ! npx_ready; then
		warn "Install Node.js LTS and npm, then rerun to configure Chrome DevTools MCP."
		return 0
	fi
