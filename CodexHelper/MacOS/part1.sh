...
brew_formula_installed() {
	local brew_path
	brew_path="$(resolve_brew 2>/dev/null || true)"
	[[ -n "$brew_path" ]] && HOMEBREW_NO_AUTO_UPDATE=1 "$brew_path" list --formula --versions "$1" >/dev/null 2>&1
}

uv_tool_installed() {
	have uv && uv tool list 2>/dev/null | awk -v package="$1" '$1 == package { found=1 } END { exit !found }'
}

pipx_package_installed() {
	have pipx && pipx list --short 2>/dev/null | awk -v package="$1" '$1 == package { found=1 } END { exit !found }'
}
	...
	if have_brew; then
		...
		if [[ "$FORCE" == "1" ]]; then
			if run_brew update; then
				record_install_ok "Homebrew package metadata refreshed"
			else
				warn "Homebrew update failed; continuing with the current package metadata."
				record_install_failed "Homebrew package metadata refresh failed"
			fi
		fi
	...
	if have uv; then
		if [[ "$FORCE" == "1" ]]; then
			if brew_formula_installed uv; then
				run_brew upgrade uv || {
					record_install_failed "uv refresh failed via Homebrew"
					return 1
				}
			elif pipx_package_installed uv; then
				run pipx pipx upgrade uv || {
					record_install_failed "uv refresh failed via pipx"
					return 1
				}
			elif ! run uv uv self update; then
	...
	if have pipx; then
		if [[ "$FORCE" == "1" ]]; then
			if brew_formula_installed pipx; then
				run_brew upgrade pipx || {
					record_install_failed "pipx refresh failed via Homebrew"
					return 1
				}
				record_install_ok "pipx refreshed via Homebrew"
			else
				warn "pipx is installed but is not owned by Homebrew; leaving it unchanged."
...
maybe_install_rtk() {
	if have_cli rtk; then
		...
		if [[ "$FORCE" == "1" ]]; then
			if brew_formula_installed rtk; then
				run_brew upgrade rtk || {
					record_install_failed "RTK refresh failed via Homebrew"
					return 1
				}
				rtk_is_expected || {
				...
				record_install_ok "RTK refreshed via Homebrew"
			else
				warn "RTK is installed but is not owned by Homebrew; leaving it unchanged."
				record_install_skipped "RTK refresh skipped: unmanaged installation"
			fi
		fi
	...
	warn "brew install finished, but the expected rtk-ai executable was not found on PATH."
...
bootstrap_prereqs() {
	...
	if [[ "$FORCE" == "1" || "$INSTALL_BREW" == "1" || "$INSTALL_PREREQS" == "1" ]]; then
		maybe_install_homebrew || true
	...
	if [[ "$FORCE" == "1" || "$INSTALL_UV" == "1" || "$INSTALL_PREREQS" == "1" ]] || ! have uv; then
		...
		if binary_in_active_venv "$binary"; then
			...
			if [[ "$FORCE" == "1" ]]; then
				if uv_tool_installed "$package"; then
					run uv uv tool upgrade "$package" || {
...
install_global_tools() {
	...
	if [[ "$FORCE" == "1" ]] && have_cli graphify; then
		if uv_tool_installed graphifyy; then
			run uv uv tool upgrade graphifyy || record_install_failed "graphifyy refresh failed via uv"
		elif pipx_package_installed graphifyy; then
			run pipx pipx upgrade graphifyy || record_install_failed "graphifyy refresh failed via pipx"
		else
