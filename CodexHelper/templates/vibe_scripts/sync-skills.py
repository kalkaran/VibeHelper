#!/usr/bin/env python3
# CodexHelper-Version: 2026-09-07-v20
...
def inspect_skill(name: str, data: dict[str, Any], provider: str) -> Finding | None:
    ...
    install_action = provider_data.get("installCommand")
    update_action = provider_data.get("updateCommand") or install_action
    if path is None:
        return Finding(
            "skill",
            name,
    ...
    return Finding(
        "skill", name, provider, "ok", f"{path} installed", update_action, policy
    )
...
def inspect_plugin(name: str, data: dict[str, Any], provider: str) -> Finding | None:
    ...
    if (
        ...
        return Finding("plugin", name, provider, "manual", note, install_action, policy)
    note = provider_data.get(
        "manualNote", "plugin install may require interactive trust"
    )
    return Finding("plugin", name, provider, "missing", note, install_action, policy)
...
def should_update(finding: Finding, force: bool, allow_manual: bool) -> bool:
    if finding.action is None:
        return False
    if force:
        return finding.status in {"missing", "drift", "ok", "manual"}
...
def main() -> int:
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Refresh every managed item, including installed and manual-policy items.",
    )
    parser.add_argument(
        "--allow-manual",
        action="store_true",
